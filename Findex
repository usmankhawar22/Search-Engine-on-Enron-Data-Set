...
###Forward indexing has one main dictionary Forwardindex which contains each document doic-id starting from 0, in each dictionary a sub dictionary contains forwardIndex[i] = [[tempSender, tempReciever, tempSubject, tempDate]]
###At each parsing of file, it is appended to main dictionary along with the word count of each word in the above mentioned format


ps = PorterStemmer()
dictionary=dict()
emailFolder = "C:/Users/user/Desktop/DSA proj/mail"
indexFolder = "C:/Users/Ahmad Naeem Khokhar/Desktop/Data Structures and Algorithms/DSA proj"
...
#for jo in range(1000):
        ...
        """for x in dis:
                ps.stem(x)
                if x not in stop_word and x not in distinct:
                        #print(x)
                        distinct.append(x)"""
        ...
        forwardIndex[i] = [[tempSender, tempReciever, tempSubject, tempDate]]
        ...
        forwardIndex[i].append(dictionary)
